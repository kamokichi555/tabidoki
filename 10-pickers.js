...
import { LS_HIGHWAY, LS_MICHI, MS_WEEK } from './00-constants.js';
...
export async function _fetchHighwayOnline(){
  return _fetchFacilityOnline({
    ...
    cacheKey:LS_HIGHWAY, minCount:30,
...
(function _initHighwayData(){
  const cached=_readFacilityCache(LS_HIGHWAY,MS_WEEK,30);
  if(cached){
...
export async function _fetchMichiOnline(){
  return _fetchFacilityOnline({
    ...
    cacheKey:LS_MICHI, minCount:100,
    query:`[out:json][timeout:100];
...
(function _initMichiData(){
  const cached=_readFacilityCache(LS_MICHI,MS_WEEK,100);
  if(cached){
