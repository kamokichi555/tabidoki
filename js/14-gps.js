...
export function _gpsOnPosition(pos){
  ...
  const _dt=_gpsLastFixTs?(_gpsLastPos.ts-_gpsLastFixTs):0;
  if(_gpsLastPos.ts-_gpsLastDbgFixTs>=15000){
    _dbgLog('gps_fix',{dt:_dt,acc:Math.round(acc)});
    _gpsLastDbgFixTs=_gpsLastPos.ts;
  }
  ...
  // 近距離タイ対策: 往復(自宅→…→自宅 等)で出発と帰着が同じ場所だと、GPSジッタで終点側が
  // 数m近いだけで「最後のaへ飛ぶ」誤爆が起きる。距離差が GPS_TIE_M 以内なら同距離とみなし、
  // インデックスが小さい方（行程の先）を優先することで、出発側を選び続ける。
  const GPS_TIE_M=40;
  ...
  for(let i=startI;i<flat.length;i++){
    ...
    if(nearestIdx===-1||d<nearestDist-GPS_TIE_M){nearestDist=d;nearestIdx=i;}
  }
