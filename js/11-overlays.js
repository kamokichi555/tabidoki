...
import { _dbgEnabled, _dbgRefreshSettings } from './12-debug.js';


/* ══ 給油チェック ══ */
export function _setFuelCheck(val){
...
export function toggleFuelCheck(){
  const box=document.getElementById('fuel-check-box');
  if(!box) return;
...
/* ══ 詳細セクション開閉 ══ */
export function _setDetailsOpen(open){
...
/* ══ スプラッシュ タイトル設定 ══ */
export const SPLASH_SK='touring_splash_settings';
export const KANJI_NUMS=['一','二','三','四','五','六','七','八','九','十'];
export function _toKanjiNum(n){
  n=parseInt(n,10);
...
export function _randomKanji(){return '第'+KANJI_NUMS[Math.floor(Math.random()*KANJI_NUMS.length)]+'話';}
export function _randomTitle(){return MONJIROU_TITLES[Math.floor(Math.random()*MONJIROU_TITLES.length)];}
export function _renderSplash(){
...
export function openSplashSettings(){
  _closeAllOverlays();
...
export function _updateEpisodePreview(){
...
export function _makeApplyVp(ov){
  return function(){
...
export function _bindOverlayVp(ov,applyVp){
...
export function _closeAllOverlays(except){
